...
| 你想要的效果 | 关总开关（BoxJs） | 在 Surge 里停用/删除模块 |
| --- | --- | --- |
| 不再修改任何请求头 | ✅ 够了 | ✅ |
| 不再收到任何通知 | ✅ 够了（v1.4.4 起，关闭后每日自检也静默） | ✅ |
| 不再占用「每个请求只跑一个 http-request 脚本」的名额 | ❌ 仍占用 | ✅ |
| `[Host]`（`vibeheader.* → 192.0.2.1`）不再生效 | ❌ 仍生效 | ✅ |
| 三条 `vibeheader.*` 不再进 MITM 列表 | ❌ 仍在列表里 | ✅ |
| 随时恢复使用 | ✅ 打开开关即恢复 | 需重新安装 |

**推荐做法：**

- **只是暂时不用** → 关总开关就够了，随时打开即恢复，不用重装任何东西。
- **确定不再用** → 在 Surge →「模块」里把 **VibeHeader（请求头管理器）** 和 **VibeHeader 信息面板** 都删掉（向左滑 →「删除」）。BoxJs 里的订阅留着也没影响，想清干净就一并删掉。
- ⚠️ 如果你**之前把「手工版」那两行 `[Script]` / `[Panel]` 贴进了自己的配置**，记得一起删掉 —— 那两行不受模块开关控制。
- 若你为此改过自己的 `[MITM]` hostname 列表（加了 `*.ess.tencent.cn` 之类的域名），按需自行清理。
